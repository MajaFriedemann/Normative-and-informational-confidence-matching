...
jsPsych.plugins['jspsych-IST-forced'] = (function () {
  var plugin = {};

  plugin.info = {
    name: 'jspsych-IST-forced',
    description: 'Forced-choice version of the Information Sampling Task (IST) with confidence sampling options',
    parameters: {
      trial_type: {
      ...
      pCorrect: {
        type: jsPsych.plugins.parameterType.OBJECT,
        pretty_name: 'sequence of decision-point P(correct) values',
        default: [],
        description: 'ARRAY/STRING: specifies the decision-point P(correct) values that we\'ll be choosing from in the forced-choice sequence. If array, will use the array as the P(correct) sequence. If string, will call existing FW/DW P(correct) sequence from dataObject.'
      },
      minTilesSeen: {
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'minimum number of tiles seen in forced choice',
        default: 1,
        description: 'INTEGER: specifies the minimum number of tiles to be seen in any forced choice trial.'
      }
    }
  };
  ...
  plugin.trial = function (display_element, trial) {
    ...
    if (Array.isArray(trial.pCorrect)) {
      var pCorrect_seq = trial.pCorrect;
    } else {
      if (trial.pCorrect == "FW") {
        var pCorrect_seq = dataObject['IST_FW_pCorrect'];
      } else if (trial.pCorrect == "DW") {
        var pCorrect_seq = dataObject['IST_DW_pCorrect'];
      }
    }
    console.log('P(correct) sequence: ' + pCorrect_seq);


    if (trial.score_rule == 'DW') {
      if (dataObject['IST_DW_totalSeen'].length > 0) {
        var minTilesSeen = math.min(dataObject['IST_DW_totalSeen']);
      } else {
        var minTilesSeen = trial.minTilesSeen;
      }
    } else {
      if (dataObject['IST_FW_totalSeen'].length > 0) {
        var minTilesSeen = math.min(dataObject['IST_FW_totalSeen']);
      } else {
        var minTilesSeen = trial.minTilesSeen;
      }
    }
    console.log('minimum number of tiles seen: ' + minTilesSeen);


    $(document).ready(resetGameboard_forced(
      display_element,
      display_element,
      trial.trial_type,
      trial.score_rule,
