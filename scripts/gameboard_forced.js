/************* FORCED-CHOICE GAMEBOARD FUNCTION ****************/
...
function resetGameboard_forced(container, parent, trialType, rule, trialCount, trialCounterVariable, difficultyLevel, colorArray, showBoard, isTutorialMode, trialDataVariable, confidenceFrequency, confidenceTrialArray, showScoreboard, showRule, feedbackFreq, showScore, showCumulative, pCorrect_seq, minTilesSeen) {

  // basic gameboard information
  trialDataVariable['IST_trialType'].push(trialType);
  trialDataVariable['IST_scoreRule'].push(rule);
  trialDataVariable['IST_isForcedChoice'].push(true);
  ...
  // P(correct) thresholds
  var randomisedPCorrect = shuffle(pCorrect_seq);
  // note: the following functions do not use the built-in Math object, but rather than mathjs package called as an external script and so MUST be specified with the lowercase 'math'.
  var mean_pCorrect = math.mean(pCorrect_seq);
  var sd_pCorrect = math.std(pCorrect_seq);

  if (randomisedPCorrect.length == trialCount) {
    var useRandomised = false;
  } else {
    var useRandomised = true;
  }
    ...
    switch(confidenceFrequency) {
      case 'randomInt':
        if (lastSelection) {
          ...
          $('.scoreboard, .response-button').removeClass('hidden');

          if (isForcedChoice == false) {
            $('.gameboard, .scoreboard, .response-button').css('pointer-events', 'auto');
          } else {
            var pCorrect = getPCorrect((majoritySeen + minoritySeen), majoritySeen);
            if (useRandomised) {
              if ((pCorrect >= (mean_pCorrect + p5.prototype.randomGaussian(mean_pCorrect, sd_pCorrect))) && (tileCount >= minTilesSeen)) {
                console.log('Trigger forced choice');
                setTimeout(function () { showForcedChoice(); }, 1);
              } else {
                $('.gameboard').css('pointer-events', 'auto');
                $('.scoreboard, .response-button').css('pointer-events', 'none');
              }
            } else {
              if ((pCorrect >= randomisedPCorrect[trialCounterVariable]) && (tileCount >= minTilesSeen)) {
                console.log('Trigger forced choice');
                setTimeout(function () { showForcedChoice(); }, 1);
              } else {
                $('.gameboard').css('pointer-events', 'auto');
      ...
      case 'tile':
        ...
        $('.response-button').removeClass('hidden');
  ...
  function endTrial() {
    ...
    $('#score').removeClass('glowy-text-white');
    ...
    setTimeout(function () {
      ...
      if (trialCounterVariable < trialCount) {
        ...
        resetGameboard_forced(container, parent, trialType, rule, trialCount, trialCounterVariable, difficultyLevel, colorArray, showBoard, isTutorialMode, trialDataVariable, confidenceFrequency, confidenceTrialArray, showScoreboard, showRule, feedbackFreq, showScore, showCumulative, pCorrect_seq, minTilesSeen);
      } else {
  ...
  function showForcedChoice() {
    // prevent further gameboard clicks
    $('.gameboard').css('pointer-events', 'none');

    // enable buttons
    $('.response-button').removeClass('nonselectable');
    $('.response-button').css('pointer-events', 'auto');
    console.log("Buttons enabled now");

    // add colours back to response buttons
    $('#left-response').addClass('leftColor');
    $('#right-response').addClass('rightColor');

    // change scoreboard text
    document.getElementById('scoreboard-text').innerHTML = '';
    document.getElementById('score').innerHTML = 'DECIDE <br><br><br> NOW';
    $('#score').addClass('glowy-text-white');
  }
  ...
  if (showRule) {
    ...
    if (isForcedChoice) {
      var scoreboardCaption = 'FORCED CHOICE MODE'
    } else {
  ...
  if (leftRightRandomizer > 0.5) {
    left_response.setAttribute('class', 'response-button MAJ');
  } else {
    right_response.setAttribute('class', 'response-button MAJ');
  }
  ...
  $('.tile').on('click', function(event) {
    ...
    // check if the ending should be forced
    var pCorrect = getPCorrect((majoritySeen + minoritySeen), majoritySeen);
